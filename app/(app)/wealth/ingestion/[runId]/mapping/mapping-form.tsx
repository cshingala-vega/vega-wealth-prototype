...
const REQUIRED_FIELDS = [
  { key: "transactionId", label: "Transaction ID / Reference" },
...
export function MappingForm({ run, templates, previewRows }: MappingFormProps) {
  ...
  const [templateName, setTemplateName] = useState(
    `${run.distributorName} — ${new Date(run.closeCycle + "-01").toLocaleDateString("en-GB", { month: "long", year: "numeric" })}`,
  )
    ...
    async function loadSuggestions() {
      try {
        ...
        for (const s of data.mappings) {
          if (s.suggestion) initial[s.field] = s.suggestion
        }
        setMapping(initial)
      } catch {
  ...
  function getSuggestion(field: string): AiSuggestion | undefined {
  ...
  function loadTemplate(templateId: string) {
    ...
    if (!tpl) return
    // template mappingJson is { sourceCol: schemaField } — invert to { schemaField: sourceCol }
    const inverted: Record<string, string> = {}
    for (const [src, schema] of Object.entries(tpl.mappingJson)) {
      inverted[schema] = src
    }
    setMapping(inverted)
  }

  // Detect duplicate source column mappings
  function getDuplicateWarning(field: string): string {
    const src = mapping[field]
    if (!src) return ""
    const others = ALL_FIELDS.filter((f) => f.key !== field && mapping[f.key] === src)
    if (others.length > 0) return `This column is already mapped to ${others[0].label}.`
    return ""
  }

  // Live preview: apply current mapping to preview rows
  const mappedPreview = previewRows.map((row) => {
    const out: Record<string, string> = {}
    for (const f of ALL_FIELDS) {
      const src = mapping[f.key]
      out[f.label] = src ? (row[src] ?? "") : ""
    }
    return out
  })
  ...
  async function handleSubmit() {
    ...
    for (const f of REQUIRED_FIELDS) {
      if (!mapping[f.key]) errors[f.key] = "Required field must be mapped"
    }
    ...
    if (result.success) {
      router.push(`/wealth/ingestion/${run.id}/review`)
    }
  }
    ...
    <PageWrapper>
      ...
      <PageContent className="space-y-6">
        ...
        {aiUnavailable && (
          <Alert className="flex items-center gap-2 border-amber-500/30 bg-amber-500/10 text-amber-700">
            ...
            <span className="text-sm">
              AI suggestions unavailable — please map columns manually.
            </span>
          </Alert>
        )}

        <div className="grid grid-cols-5 gap-4">
          {/* Left: Mapping table (60%) */}
          <div className="col-span-3 space-y-4">
            ...
            {aiLoading ? (
              <div className="flex items-center gap-2 py-8 text-muted-foreground">
                ...
                <span className="text-sm">Getting AI column suggestions…</span>
              </div>
            ) : (
              <Card>
                <CardHeader>
                ...
                <CardContent className="p-0">
                  <Table>
                    <TableHeader>
                      <TableRow>
                        <TableHead>Vega Schema Field</TableHead>
                    ...
                    <TableBody>
                      {REQUIRED_FIELDS.map((f) => {
                        const sug = getSuggestion(f.key)
                        const dup = getDuplicateWarning(f.key)
                          ...
                          >
                            ...
                            <TableCell>
                              <div className="space-y-1">
                                ...
                                {dup && !hasError && (
                                  <p className="text-xs text-amber-600">{dup}</p>
                                )}
                              </div>
                            </TableCell>
                            <TableCell>
                          ...
                          <TableRow key={f.key}>
                            ...
                            <TableCell>
                              <div className="space-y-1">
                                ...
                                {dup && <p className="text-xs text-amber-600">{dup}</p>}
                              </div>
                            </TableCell>
                          </TableRow>
                        )
            ...
            <Card>
              <CardContent className="py-4 space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  />
                  <label htmlFor="save-tpl" className="text-sm font-medium cursor-pointer">
                    Save as template
                  </label>
                </div>
                {saveTemplate && (
        ...
        <div className="flex justify-end gap-3">
          <Button
            variant="outline"
            onClick={() => router.push(`/wealth/ingestion/new`)}
          >
            ← Back
          </Button>
          <Button onClick={() => void handleSubmit()} disabled={submitting || aiLoading}>
            {submitting ? "Saving…" : "Run Ingestion →"}
          </Button>
        </div>
      </PageContent>
    </PageWrapper>
