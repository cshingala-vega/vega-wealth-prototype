...
interface ReconMappingFormProps {
  run: RunInfo
  templates: Template[]
...
export function ReconMappingForm({ run, templates, previewRows }: ReconMappingFormProps) {
  ...
  const [templateName, setTemplateName] = useState(
    `${run.transferAgentName} \u2014 ${new Date(run.closeCycle + "-01").toLocaleDateString("en-GB", { month: "long", year: "numeric" })}`,
  )
    ...
    async function load() {
      try {
        ...
        for (const s of data.mappings) {
          if (s.suggestion) init[s.field] = s.suggestion
        }
        setMapping(init)
      } catch {
  ...
  function getSuggestion(field: string) {
  ...
  function loadTemplate(tid: string) {
    ...
    if (!tpl) return
    // Template mappingJson is { sourceCol: schemaField } — invert
  ...
  // Build live preview rows using current mapping
  ...
  async function handleSubmit() {
    ...
    for (const f of REQUIRED_FIELDS) {
      if (!mapping[f.key]) errors[f.key] = "Required"
    }
    ...
    setSubmitting(true)
    const result = await saveReconciliationMapping({
      runId: run.id,
      mapping,
      saveTemplate,
      templateName,
    ...
    setSubmitting(false)
    if (result.success) {
      router.push(`/wealth/reconciliation/${run.id}/results`)
    }
  }
    ...
    <PageWrapper>
      ...
      <PageContent className="space-y-6">
            ...
            <Card>
              <CardContent className="space-y-3 py-4">
                <div className="flex items-center gap-2">
                  ...
                  />
                  <label htmlFor="save-tpl-r" className="cursor-pointer text-sm font-medium">
                    Save as template
                  </label>
                </div>
                {saveTemplate && (
        ...
        <div className="flex justify-end gap-3">
          <Button
            variant="outline"
            onClick={() => router.push("/wealth/reconciliation/new")}
          >
            Back
          </Button>
          <Button
